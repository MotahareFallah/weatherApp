...
function getIcon(weCondition) {
  let icon;
  switch (weCondition) {
    case "Thunderstorm":
      icon = `${weCondition}.png`;
      break;
    case "Drizzle":
      icon = `${weCondition}.png`;
      break;
    ...
    case "Clear":
      ...
      icon = `${weCondition}-${DayOrNigh}.png`;
      break;
    case "Clouds":
      icon = `${weCondition}.png`;
      break;
...
window.addEventListener("load", function () {
  getLoc().then((locData) => {
    ...
    getWeather(locData.lat, locData.lon).then((weData) => {
      ...
      const weCondition = weData.weather[0].main;
      const weDes = weData.weather[0].description;

      const iconName = getIcon(weCondition);
      ...
      console.log(weTemp, weCondition, weDes);
    });
  });
});
